...
func (h *TitleFilterHandler) RegisterHandler(engine *gin.RouterGroup) {
	...
	engine.GET("/title-filters/active", h.listActiveFilters)
...
// listActiveFilters 供客户端发布流程拉取全部有效规则。
func (h *TitleFilterHandler) listActiveFilters(c *gin.Context) {
	result, err := h.service.ListActiveFilters()
	commonRouter.ToJson(c, result, err)
}
