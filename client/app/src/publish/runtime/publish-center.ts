...
class PublishCenterStore {
  ...
  upsertFromTaskRecord(task: PublishTaskRecord, overrides?: Partial<PublishRuntimeTaskSnapshot>): void {
    ...
    const next: PublishRuntimeTaskSnapshot = {
      ...
      waitingForLogin: overrides?.waitingForLogin ?? previous?.waitingForLogin ?? false,
      loginRequiredShopId: overrides?.loginRequiredShopId ?? previous?.loginRequiredShopId,
  ...
  applyProgressEvent(taskId: number, event: PublishProgressEvent): void {
    ...
    const next: PublishRuntimeTaskSnapshot = {
      ...
      waitingForLogin: event.status === StepStatus.PENDING && Boolean(event.loginRequiredShopId),
      loginRequiredShopId: event.loginRequiredShopId ?? previous?.loginRequiredShopId,
